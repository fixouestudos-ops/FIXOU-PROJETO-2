...
CREATE TABLE IF NOT EXISTS user_progress (user_id TEXT PRIMARY KEY,state_json TEXT NOT NULL,client_saved_at INTEGER NOT NULL DEFAULT 0,server_revision INTEGER NOT NULL DEFAULT 1,updated_at INTEGER NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS analytics_events (id TEXT PRIMARY KEY,user_id TEXT,event_type TEXT NOT NULL,event_key TEXT,metadata_json TEXT NOT NULL DEFAULT '{}',created_at INTEGER NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE SET NULL);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_events_user_key ON analytics_events(user_id,event_key) WHERE event_key IS NOT NULL;
