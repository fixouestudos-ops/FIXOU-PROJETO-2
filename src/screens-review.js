...
function reviewQuestionCard(q,index,total){
 ...
 return `<div class="review-card" data-question-id="${escapeHTML(q.id)}">
  ...
  <div class="review-card-meta">
   ...
   ${q.sourceBook?`<span><strong>Livro:</strong> ${escapeHTML(q.sourceBook)}</span>`:''}
   ${q.sourcePage?`<span><strong>Página:</strong> ${q.sourcePage}</span>`:''}
   ${q.questionNumber?`<span><strong>Nº:</strong> ${q.questionNumber}</span>`:''}
  </div>
...
export function reviewScreen({bank,reviewFilter='all',reviewSearch='',reviewIndex=0}){
 ...
 if(reviewFilter==='image_required')filtered=questions.filter(q=>q.imageClassification==='image_required');
 else if(reviewFilter==='no_image_required')filtered=questions.filter(q=>q.imageClassification==='no_image_required');
 else if(reviewFilter==='needs_visual_review')filtered=questions.filter(q=>q.imageClassification==='needs_visual_review'||q.imageStatus==='needs_visual_review');
 else if(reviewFilter==='approved')filtered=questions.filter(q=>q.imageStatus==='approved'||q.imageStatus==='approved_manual');
 else if(reviewFilter==='needs_content_review')filtered=questions.filter(q=>q.contentStatus==='needs_content_review');

 if(reviewSearch){
  const s=reviewSearch.toLowerCase();
  filtered=filtered.filter(q=>q.id.toLowerCase().includes(s)||(q.prompt||'').toLowerCase().includes(s)||(q.sourceBook||'').toLowerCase().includes(s)||String(q.sourcePage||'').includes(s)||String(q.questionNumber||'').includes(s));
 }

 const safeIndex=Math.min(reviewIndex,Math.max(0,filtered.length-1));
 const current=filtered[safeIndex];

 const counts={
  all:questions.length,
  image_required:questions.filter(q=>q.imageClassification==='image_required').length,
  no_image_required:questions.filter(q=>q.imageClassification==='no_image_required').length,
  needs_visual_review:questions.filter(q=>q.imageClassification==='needs_visual_review'||q.imageStatus==='needs_visual_review').length,
  approved:questions.filter(q=>q.imageStatus==='approved'||q.imageStatus==='approved_manual').length,
  needs_content_review:questions.filter(q=>q.contentStatus==='needs_content_review').length
 };
 ...
 <div class="review-nav">
  ...
  <span class="review-nav-info">${filtered.length>0?`${safeIndex+1} de ${filtered.length}`:'Nenhuma questão encontrada'}</span>
  <button class="review-nav-btn" data-action="review-next" ${safeIndex>=filtered.length-1?'disabled':''}>Próxima →</button>
 </div>
 ${current?reviewQuestionCard(current,safeIndex,filtered.length):'<div class="review-empty">Nenhuma questão encontrada para este filtro.</div>'}
